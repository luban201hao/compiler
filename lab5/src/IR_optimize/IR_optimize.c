...
void IR_optimize() {
    ...
    // 在IR_optimize文件夹下面，
    // available_expression_analysis.c, constant_propagation.c, 
    // copy_propagation.c和live_variable_analysis.c四个文件都是DataflowAnalysis的实现类

    for_vec(IR_function_ptr, i, ir_program_global->functions) {
        // 此处的函数指的是IR文件中定义的一个函数
        IR_function *func = *i;
